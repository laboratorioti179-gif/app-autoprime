...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 3. Busca de dados em paralelo de forma independente para abrir a tela imediatamente
      supabase.from('autoprime_profile').select('*').eq('tenant_id', currentTenantId).maybeSingle().then(({ data: pData, error: pErr }) => {
        if (!pErr && pData) {
          setProfile(pData);
      ...
      supabase.from('autoprime_crm').select('*').eq('tenant_id', currentTenantId).order('last_entry', { ascending: false }).then(({ data, error }) => { if (!error && data) setCrmData(data); });
      supabase.from('autoprime_inventory').select('*').eq('tenant_id', currentTenantId).order('created_at', { ascending: false }).then(({ data, error }) => { if (!error && data) setInventory(data); });
      supabase.from('autoprime_inventory_log').select('*').eq('tenant_id', currentTenantId).order('created_at', { ascending: false }).then(({ data, error }) => { if (!error && data) setInventoryLog(data); });
      supabase.from('autoprime_fixed_costs').select('*').eq('tenant_id', currentTenantId).maybeSingle().then(({ data, error }) => { if (!error && data) setFixedCosts({ ...data, custom_costs: Array.isArray(data.custom_costs) ? data.custom_costs : [] }); });
